    ...
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        if file_path.endswith('.csv'):
            ...
            with h5py.File(file_path, 'r') as h5_file:
                function_sources = h5_file['functionSource']
                labels_group = h5_file['CWE-other'] 

                for i in tqdm(range(len(function_sources))):
                    func_source = function_sources[i].decode('utf-8')
                    label = int(labels_group[i])
                    self.examples.append(convert_examples_to_features(func_source, label, tokenizer, args))
        if file_type == "train":
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    if args.do_local_explanation:
        for reasoning_method in all_reasoning_method:
            ...
            logger.info(f"NA Eval Results (Out of 256 Tokens): {na_eval_results_256}")
    ...
    if do_explanation:
        if reasoning_method == "attention":
            ...
            if reasoning_method == "deeplift":
                #baselines = torch.randn(1, 256, 768, requires_grad=True).to(args.device)
                baselines = torch.zeros(1, 256, 768, requires_grad=True).to(args.device)
                reasoning_model = DeepLift(model)
            elif reasoning_method == "deeplift_shap":
                #baselines = torch.randn(16, 256, 768, requires_grad=True).to(args.device)
                baselines = torch.zeros(16, 256, 768, requires_grad=True).to(args.device)
